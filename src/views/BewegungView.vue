...
const bewegungItems = [
  ...
  { id: 'drehen', text: 'drehen', type: 'movement' },
  { id: 'legen', text: 'legen', type: 'movement' },
  { id: 'stuetzen', text: 'stützen', type: 'movement' },
  { id: 'schieben', text: 'schieben', type: 'movement' },
  
  // Zeile 2: Arm, Bein, Kopf, Körper, Bett
  { id: 'arm', text: 'Arm', type: 'bodypart' },
  { id: 'bein', text: 'Bein', type: 'bodypart' },
  { id: 'kopf', text: 'Kopf', type: 'bodypart' },
  { id: 'koerper', text: 'Körper', type: 'bodypart' },
  { id: 'bett', text: 'Bett', type: 'object' },
  
  // Zeile 3: rechts, links, oben, unten, vorne, hinten
  { id: 'rechts', text: 'rechts', type: 'direction' },
  { id: 'links', text: 'links', type: 'direction' },
  { id: 'oben', text: 'oben', type: 'direction' },
  { id: 'unten', text: 'unten', type: 'direction' },
  { id: 'vorne', text: 'vorne', type: 'direction' },
  { id: 'hinten', text: 'hinten', type: 'direction' },
  
  // Zeile 4: anwinkeln, zur Seite
  { id: 'anwinkeln', text: 'anwinkeln', type: 'movement' },
  { id: 'zur_seite', text: 'zur Seite', type: 'direction' },
    ...
    <main class="flex-1 flex items-center justify-center p-16">
      <div class="max-w-8xl mx-auto">
        ...
        <div class="space-y-20 mt-32 mb-48">
          ...
          <div class="flex justify-center space-x-32">
            <button
              v-for="(item, index) in bewegungItems.slice(4, 9)"
              :key="item.id"
              @click="selectBewegung(item.id)"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
          ...
          <div class="flex justify-center space-x-32">
            <button
              v-for="(item, index) in bewegungItems.slice(9, 15)"
              :key="item.id"
              @click="selectBewegung(item.id)"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
          ...
          <div class="flex justify-center space-x-32">
            <button
              v-for="(item, index) in bewegungItems.slice(15, 17)"
              :key="item.id"
              @click="selectBewegung(item.id)"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
                fontSize: '2.646rem',
                background: currentTileIndex === index + 15 ? '#f3f4f6' : 'white',
                border: '2px solid #d1d5db',
                borderRadius: '15px',
                outline: 'none',
                boxShadow: 'none',
              ...
              :class="currentTileIndex === index + 15 ? 'text-orange-500 scale-110' : 'text-black hover:text-gray-600'"
            >
              {{ item.text }}
            </button>
          </div>
          ...
          <div class="flex justify-center">
            <button
              @click="selectBewegung('zurueck')"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
                fontSize: '2.646rem',
                background: currentTileIndex === 17 ? '#f3f4f6' : 'white',
                border: '2px solid #d1d5db',
                borderRadius: '15px',
                outline: 'none',
                boxShadow: 'none',
              ...
              :class="currentTileIndex === 17 ? 'text-orange-500 scale-110' : 'text-black hover:text-gray-600'"
            >
              {{ bewegungItems[17].text }}
            </button>
          </div>
        </div>
