...
const ernaehrungItems = [
  // Hauptkategorien
  { id: 'essen', text: 'Essen', type: 'kategorie' },
  { id: 'trinken', text: 'Trinken', type: 'kategorie' },
  
  // Essen-Unterkategorien
  { id: 'suess', text: 'süß', type: 'essen' },
  { id: 'herzhaft', text: 'herzhaft', type: 'essen' },
  { id: 'scharf', text: 'scharf', type: 'essen' },
  { id: 'kalt', text: 'kalt', type: 'essen' },
  { id: 'warm', text: 'warm', type: 'essen' },
  { id: 'lauwarm', text: 'lauwarm', type: 'essen' },
  { id: 'trocken', text: 'trocken', type: 'essen' },
  { id: 'nass', text: 'nass', type: 'essen' },
  { id: 'breiig', text: 'breiig', type: 'essen' },
  
  // Trinken-Items
  { id: 'wasser', text: 'Wasser', type: 'trinken' },
  { id: 'saft', text: 'Saft', type: 'trinken' },
  { id: 'milch', text: 'Milch', type: 'trinken' },
  ...
  <div class="min-h-screen bg-white dark:bg-gray-900 flex flex-col">
    ...
    <main class="flex-1 flex items-center justify-center p-16">
      <div class="max-w-8xl mx-auto">
        ...
        <!-- Ernährung-Items Tastatur -->
        <div class="space-y-20 mt-32 mb-48">
          <!-- Zeile 1: Essen, Trinken -->
          <div class="flex justify-center space-x-32">
            <button
              v-for="(item, index) in ernaehrungItems.slice(0, 2)"
              :key="item.id"
          ...
          <!-- Zeile 2: süß, herzhaft, scharf -->
          <div class="flex justify-center space-x-32">
            <button
              v-for="(item, index) in ernaehrungItems.slice(2, 5)"
              :key="item.id"
              @click="selectErnaehrung(item.id)"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
                fontSize: '2.646rem',
                background: currentTileIndex === index + 2 ? '#f3f4f6' : 'white',
              ...
              :class="currentTileIndex === index + 2 ? 'text-orange-500 scale-110' : 'text-black hover:text-gray-600'"
            >
              {{ item.text }}
            </button>
          </div>

          <!-- Zeile 3: kalt, warm, lauwarm -->
          <div class="flex justify-center space-x-32">
            <button
              v-for="(item, index) in ernaehrungItems.slice(5, 8)"
              :key="item.id"
              @click="selectErnaehrung(item.id)"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
          ...
          <!-- Zeile 4: trocken, nass, breiig -->
          <div class="flex justify-center space-x-32">
            <button
              v-for="(item, index) in ernaehrungItems.slice(8, 11)"
              :key="item.id"
              @click="selectErnaehrung(item.id)"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
          ...
          <!-- Zeile 5: Wasser, Saft, Milch -->
          <div class="flex justify-center space-x-32">
            <button
              v-for="(item, index) in ernaehrungItems.slice(11, 14)"
              :key="item.id"
              @click="selectErnaehrung(item.id)"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
                fontSize: '2.646rem',
                background: currentTileIndex === index + 11 ? '#f3f4f6' : 'white',
                border: '2px solid #d1d5db',
                borderRadius: '15px',
                outline: 'none',
                boxShadow: 'none',
              ...
              :class="currentTileIndex === index + 11 ? 'text-orange-500 scale-110' : 'text-black hover:text-gray-600'"
            >
              {{ item.text }}
            </button>
          </div>
          ...
          <div class="flex justify-center">
            <button
              @click="selectErnaehrung('zurueck')"
              class="transition-all duration-300 font-medium hover:scale-110"
              :style="{
                fontSize: '2.646rem',
                background: currentTileIndex === 14 ? '#f3f4f6' : 'white',
                border: '2px solid #d1d5db',
                borderRadius: '15px',
                outline: 'none',
                boxShadow: 'none',
              ...
              :class="currentTileIndex === 14 ? 'text-orange-500 scale-110' : 'text-black hover:text-gray-600'"
            >
              {{ ernaehrungItems[14].text }}
            </button>
          </div>
        </div>
