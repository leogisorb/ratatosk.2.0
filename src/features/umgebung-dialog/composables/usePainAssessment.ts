...
export function usePainAssessment() {
  ...
  // Auto mode functions - wie Pain-Dialog: TTS, dann Pause, dann Karussell
  const startAutoMode = async (items: any[], titleDelay: number = 1000, cycleDelay: number = 3000) => {
    ...
    // Pause nach TTS (wie Pain-Dialog: 3 Sekunden)
    setTimeout(() => {
      console.log('UmgebungDialog: TTS completed, starting Karussell after pause')
      cycleThroughItems(items, cycleDelay)
    }, titleDelay + 3000) // 3 Sekunden Pause nach TTS
  }
