...
export function useFaceRecognition() {
    ...
    try {
      ...
      try {
        ...
        if (videoDevices.length === 0) {
          console.warn('Keine Kameras gefunden - verwende Fallback-Modus')
          // Setze einen Fallback-Modus ohne Kamera
          isActive.value = true
          error.value = 'Keine Kamera verfügbar - Face Recognition im Fallback-Modus'
          return
        }
        ...
        for (const constraint of constraints) {
          try {
            ...
            // Spezielle Behandlung für NotFoundError
            if (err instanceof Error && err.name === 'NotFoundError') {
              console.warn('Kamera nicht gefunden - versuche nächste Konfiguration')
              continue
            }
            continue
          }
  ...
  function isBlinking(): boolean {
    ...
    if (error.value && error.value.includes('Fallback-Modus')) {
      // Simuliere gelegentliches Blinzeln für Testzwecke
      const now = Date.now()
      const blinkInterval = 5000 // Alle 5 Sekunden ein simuliertes Blinzeln
      const lastBlink = (window as any).lastSimulatedBlink || 0
      
      if (now - lastBlink > blinkInterval) {
        (window as any).lastSimulatedBlink = now
        return true
      }
      return false
    }
